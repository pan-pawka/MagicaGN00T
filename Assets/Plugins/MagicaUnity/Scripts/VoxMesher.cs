...
namespace GN00T.MagicaUnity
{
    ...
    public class VoxMesher
    {
        ...
        public VoxMesher(float scale = 1)
        {
            this.scale = scale;
        }
        ...
        public Mesh AddSegment(VoxModel data, int frame, Mesh result)
        {
            Reset();
            VoxelData grid = data.frames[frame];
                    ...
                    for (int j = 0; j < dimensions[v]; ++j)
                        for (int i = 0; i < dimensions[u];)
                        {
                            ...
                            if (maskValue != 0)
                            {
                                ...
                                dv[v] = h * scale;
                                //create the face
                                addFace(new Vector3(xp[0], xp[1], xp[2]),
                                ...
                                new Vector3(q[0], q[1], q[2]), colorList[colorValue-1],maskValue< 0, verts, normals, colors, indices);
